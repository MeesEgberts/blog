...
* **Domain** code for the `invoices` bounded context depends on domain concepts such as aggregates, value objects, and domain events, not on persistence or HTTP. In this guide, the aggregates inherit from a shared base class that adapts Nest’s eventing facilities; that base class sits at the boundary between domain and infrastructure and is a deliberate compromise for integration.
